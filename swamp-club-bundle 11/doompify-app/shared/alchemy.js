...
export async function isHolderOfCollection(
  ...
  // The endpoint's boolean field has varied across Alchemy API versions
  // (isHolderOfContract / isHolderOfCollection / isHolder). Read whichever is
  // present; if none of the known names exist but there's exactly one boolean
  // field in the response, use that. Defaults to false only when truly absent.
  let held =
  ...
  if (held === undefined) {
    ...
    if (boolKeys.length === 1) held = data[boolKeys[0]];
  }
  held = Boolean(held);
  if (process.env.VERIFY_DEBUG !== "0") {
    console.log(
      `[holder] wallet=${wallet.slice(0, 8)}… contract=${contract.slice(0, 8)}… ` +
      `raw=${JSON.stringify(data)} -> isHolder=${held}`
    );
  }
  return { isHolder: held };
}
