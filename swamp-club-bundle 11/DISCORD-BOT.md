...
| `/setup`            | **(Admins)** Post the branded DOOMPS verification panel in a channel. |
...
## Part G — Post the verification panel (the /setup embed)

Once the bot is in your server and online, drop a good-looking, DOOMPS-branded
"how to verify" panel into a channel so members know what to do:

1. Go to the channel you want the panel in (e.g. `#verify` or `#start-here`).
2. Run **`/setup`** (only server managers can use it).
3. The bot posts an acid-green embed titled **"🐸 WELCOME TO THE SWAMP"** with
   the step-by-step verify instructions and the command list. It also sends you
   a private "panel posted" confirmation.

The panel is a normal message, so it stays in the channel for everyone. Re-run
`/setup` anytime to post a fresh copy (e.g. in a new channel).

### Optional: add your artwork to the panel
By default the embed is text + the acid-green DOOMPS color. To include your
banner/logo, set these (public https image URLs) in the bot's environment and
restart:

    EMBED_IMAGE_URL=https://doomps.xyz/img/banner.jpeg      # large banner
    EMBED_THUMBNAIL_URL=https://doomps.xyz/img/logo.jpeg    # small corner logo

(Your home site already hosts these images once it's live on doomps.xyz, so
those URLs will work. Any public image URL is fine.)
...
## Fixing "Invalid redirect URI"

This error means the redirect URL the app sends to Discord isn't in your app's
allowed list. Fix it in two places:

**1. Register all three URLs in Discord.** Developer Portal → your app →
**OAuth2 → Redirects → Add Redirect**, add these exactly (your real domain,
...
**2. Make sure `PUBLIC_URL` matches.** The app builds those URLs from
`PUBLIC_URL`, so on Render set:

```
PUBLIC_URL=https://club.doomps.xyz
```

exactly — no trailing slash, `https`, and the same subdomain you registered.
(The app now strips a stray trailing slash automatically, and the three
callback URLs auto-derive from `PUBLIC_URL`, so you don't need to set the
`*_REDIRECT_URI` vars at all unless you want to override them.)

After changing either, redeploy and try the login again.
...
| **"Invalid redirect URI" / "invalid OAuth2 redirect_uri"** | The URL the app sends isn't registered in Discord. Register all three exactly (see below), and make sure `PUBLIC_URL` is exactly `https://club.doomps.xyz` (no trailing slash, https). |
